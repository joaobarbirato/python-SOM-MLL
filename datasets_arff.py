...
for dataset, n_classes in DATASET_NUMEROCLASSES:
    ...
    x = scale([data['data'][i][:-n_classes] for i in range(n_exemplos)])
    y = np.ndarray(shape=(n_exemplos, n_classes), dtype=int, buffer=np.array([data['data'][i][-n_classes:] for i in range(n_exemplos)], dtype=int))
